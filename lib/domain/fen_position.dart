enum FenActiveColor { white, black }
...
class FenPosition {
  ...
  factory FenPosition.parse(String fen) {
    ...
    for (var rankOffset = 0; rankOffset < 8; rankOffset++) {
      ...
      if (file != 8) {
        throw FormatException('FEN-Reihe $rank enthält $file statt 8 Felder');
      }
    }
    ...
    if (castling != '-' && !RegExp(r'^(K?Q?k?q?)$').hasMatch(castling)) {
  ...
  String? pieceAt(String square) => _board[_squareIndex(square)];

  bool isWhitePieceAt(String square) {
    final piece = pieceAt(square);
    return piece != null && _isWhite(piece);
  }
  ...
  void applyUci(String uci) {
    ...
    if (piece == null) {
      throw FormatException('Auf $fromSquare steht keine Figur für $uci');
    }

    final movingWhite = _isWhite(piece);
    if ((activeColor == FenActiveColor.white) != movingWhite) {
      throw FormatException('Falsche Farbe am Zug für $uci in ${toFen()}');
    }

    final targetPiece = _board[to];
    if (targetPiece != null && _isWhite(targetPiece) == movingWhite) {
      throw FormatException('Eigene Figur auf Zielfeld $toSquare bei $uci');
    }
    ...
    if (lowerPiece == 'p' &&
      ...
      if (capturedPawn != expectedPawn) {
        throw FormatException('Ungültiges En passant bei $uci');
      }
  ...
  void _updateCastlingRightsForMove({
    ...
    if (capturedPiece != null && capturedPiece.toLowerCase() == 'r') {
  ...
  void _moveCastlingRook({
    ...
    final (rookFrom, rookTo) = switch (toSquare) {
      ...
      'g8' => ('h8', 'f8'),
      'c8' => ('a8', 'd8'),
      _ => throw FormatException('Ungültiges Rochade-Zielfeld: $toSquare'),
    };
